...
func (s *LocalSnapStore) Fetch(snap brtypes.Snapshot) (io.ReadCloser, error) {
	...
	if err != nil {
		...
		prefix := path.Join(strings.Join(prefixTokens, "/"), "V1")
...
func (s *LocalSnapStore) Delete(snap brtypes.Snapshot) error {
	...
	// 	}
	// }
	//Delete v1 snap directory if empty
	if snap.SnapDir != "" {
		prefixTokens := strings.Split(s.prefix, "/")
		prefixTokens = prefixTokens[:len(prefixTokens)-1]
		prefix := path.Join(strings.Join(prefixTokens, "/"), "V1")
		err := os.Remove(path.Join(prefix, snap.SnapDir))
		if pathErr, ok := err.(*os.PathError); ok && pathErr.Err != syscall.ENOTEMPTY {
			//fmt.Printf("Error2: %s\n", err.Error())
			return err
		}
...
func (s *LocalSnapStore) isV1Present() bool {
	...
	if _, err := os.Stat(path.Join(strings.Join(prefixTokens, "/"), "V1")); os.IsNotExist(err) {
		return false
	}
	return true
}
...
func (s *LocalSnapStore) isV2Present() bool {
	...
	if _, err := os.Stat(path.Join(strings.Join(prefixTokens, "/"), "V2")); os.IsNotExist(err) {
		return false
	}
	return true
}
